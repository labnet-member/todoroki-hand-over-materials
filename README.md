...
この方法は自分の PC に IDE をインストールし，ソフトウェアの開発を行う方法である．ローカルのマシンにインストールする場合，Toolbox から指定した IDE をインストールするだけである．インストールが完了後，Toolbox の IDE を選択すると起動する．

## リモートのマシンにインストール

この方法はリモートのサーバに IDE をインストールし，ソフトウェアの開発を行う方法である．リモートにインストールする場合，まず Toolbox から [Gateway](https://www.jetbrains.com/ja-jp/remote-development/gateway/) をインストールする．Gateway はリモートマシン上に IDE をインストールし，Gateway 経由で IDE を操作することができる．ただし，この機能は Beta であり，バージョンアップによって UI が大きく変化する可能性があるため，注意が必要である．

Gateway を起動すると以下のような画面になる．<span style="color: red">赤枠の領域</span>がリモート先の選択肢である．今回は SSH を利用する．そのため，リモートのサーバに SSH 接続するための設定を事前に行っておく必要がある．これ以降は SSH の設定が既に完了していることを前提にする．
...
リモートで起動する IDE と実行したいソフトウェアを含んだディレクトリを指定して IDE を起動する．IDE は EAP（Early Access Program）以外を指定した方が良い．これが完了すると IDE が起動する．
...
まず，GoLand をソースコードが含まれているプロジェクトを指定して起動する．プロジェクトのディレクトリ構成の内，重要なもののみを以下に示す．
...
この節では提案手法を AWS ECS で検証した手順を示す．ただし，以下に示す手順は実験時（2024/2/29）で行った操作であり，このマニュアル作成時点（2024/3/16）でも正しく動作するかは保証できないため，コマンドが失敗する場合は適宜インターネットでの検索で解決するようにお願いする．`Makefile`に自動化するコマンドが載っているが，正しく動かないと思われるため，自分で自動化したいときに参考にして頂ければ．

## 環境構築

以下の環境を構築する．

![environtments](images/section_4/environments.png)
...
アクセストークン発行後`aws configure`コマンドを実行し， AWS CLI が認証時に利用する情報を登録する．これで AWS CLI の利用が可能になる．
